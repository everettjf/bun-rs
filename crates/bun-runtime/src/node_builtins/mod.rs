...
fn build_v8_stub<'ctx>(ctx: &'ctx Context) -> Value<'ctx> {
    ctx.eval(
        r#"({
            ...
            getHeapStatistics: () => ({ total_heap_size: 0, used_heap_size: 0, heap_size_limit: 0, malloced_memory: 0, peak_malloced_memory: 0, total_available_size: 0, number_of_native_contexts: 0, number_of_detached_contexts: 0, total_heap_size_executable: 0, total_physical_size: 0, used_global_handles_size: 0, total_global_handles_size: 0, external_memory: 0 }),
            ...
            getHeapSnapshot: () => new ReadableStream({ start(c) { c.enqueue(new TextEncoder().encode('{"snapshot":{"meta":{"node_fields":[],"node_types":[],"edge_fields":[],"edge_types":[]}},"nodes":[],"edges":[],"strings":[]}')); c.close(); } }),
            writeHeapSnapshot: (path) => { const fs = require("node:fs"); const p = path || "/tmp/Heap." + Date.now() + ".heapsnapshot"; fs.writeFileSync(p, '{"snapshot":{"meta":{"node_fields":[],"node_types":[],"edge_fields":[],"edge_types":[]}},"nodes":[],"edges":[],"strings":[]}'); return p; },
            cachedDataVersionTag: () => 0,
            setFlagsFromString: () => {},
            stopCoverage: () => {},
            startupSnapshot: { addDeserializeCallback: () => {}, addSerializeCallback: () => {}, isBuildingSnapshot: () => false, setDeserializeMainFunction: () => {} },
            promiseHooks: { onInit: () => () => {}, onSettled: () => () => {}, onBefore: () => () => {}, onAfter: () => () => {}, createHook: () => ({}) },
            takeCoverage: () => {},
            queryObjects: () => [],
            Serializer: class { writeHeader() {} writeValue() {} releaseBuffer() { return new Uint8Array(0); } },
            Deserializer: class { readHeader() {} readValue() {} },
