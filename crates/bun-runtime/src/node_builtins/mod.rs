...
fn build_v8_stub<'ctx>(ctx: &'ctx Context) -> Value<'ctx> {
    ctx.eval(
        r#"({
            ...
            getHeapSnapshot: () => { const json = Bun.generateHeapSnapshot("v8"); return new ReadableStream({ start(c) { c.enqueue(new TextEncoder().encode(json)); c.close(); } }); },
            writeHeapSnapshot: (path) => { const fs = require("node:fs"); const p = path || "/tmp/Heap-" + Date.now() + ".heapsnapshot"; fs.writeFileSync(p, Bun.generateHeapSnapshot("v8")); return p; },
